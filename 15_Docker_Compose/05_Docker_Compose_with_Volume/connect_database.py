...
# MYSQL_HOST = os.environ.get("MYSQL_HOST", "172.17.0.2") # uncomment for network setup
MYSQL_HOST = os.environ.get("MYSQL_HOST", "mysql_container")  # use Docker container name as hostname
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Interactive menu for adding/showing product names in newProducts.txt
        file_path = os.path.join(os.path.dirname(__file__), "newProducts.txt")
        print("\nInteractive menu:")
        print(" press 1 to insert a product name")
        print(" press 2 to show product names")
        ...
        try:
            while True:
                ...
                if choice == "1":
                    ...
                    if not name:
                        print("⚠️ Empty name, nothing added.")
                        continue
                    # Append name to file
                    os.makedirs(os.path.dirname(file_path), exist_ok=True)
                    with open(file_path, "a", encoding="utf-8") as f:
                        f.write(name + "\n")
                    print(f"✅ Added to {os.path.basename(file_path)}: {name}")
                elif choice == "2":
                    if not os.path.exists(file_path):
                        print("ℹ️ No products file found.")
                        continue
                    with open(file_path, "r", encoding="utf-8") as f:
                        lines = [ln.rstrip("\n") for ln in f if ln.strip()]
                    if not lines:
                        print("ℹ️ No products in file.")
                    else:
                        print(f"📄 Products in {os.path.basename(file_path)}:")
                        for ln in lines:
                            print("-", ln)
                elif choice.lower() == "q":
                    print("Exiting menu.")
